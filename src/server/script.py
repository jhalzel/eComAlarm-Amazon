...
def calculate_pending_sales(asin_counter, asins_list, client):
    """
    Calculate the total pending sales based on the given ASIN counter, ASINs list, and client.

    Parameters:
    asin_counter (dict): A dictionary containing the quantity of each ASIN.
    asins_list (list): A list of ASINs.
    client: The client object used to make API calls.

    Returns:
    float: The total pending sales amount.
    """
    total_sales = 0
    qty = 0
...
def main():
    ...
    if len(fbm_order_list) <= 0:
        print('fbm_order_list: No FBM orders')
